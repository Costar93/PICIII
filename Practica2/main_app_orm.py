...
path_to_database = "users.db"#s'ha d'executar a la carpeta practica2
engine = create_engine('sqlite:///'+path_to_database)

Base.metadta.bind = engine
...
def save_data(username,fullname,email,password):
    ...
    #Adding user
        try:
            session = DBSession
            nou_usuari=users(username=username, fullname=fullname, email=email, password=password)
            session.add(nou_usuari)
            session.commit
            return True
        except:
            session.rollback()
